...
```{r}
dados_cota = read_csv2("dados/valor-cota-por-estado.csv")
glimpse(dados_cota)
```

```{r, fig.width=10}
dados_cota %>%
  ggplot(aes(x=reorder(estado, valor), y=valor, fill=estado)) +
  geom_bar(stat = "identity") +
  geom_hline(yintercept = mean(dados_cota$valor))
```

```{r}
dados_cota %>%
  ggplot(aes(x= 0, y = valor)) +
  geom_boxplot()
```

```{r}
summary(dados_cota$valor)
```
...
dados_sum %>%
  ggplot(aes(x = reorder(uf, n_deputados), y = n_deputados, fill=uf)) +
  geom_bar(stat = "identity")
```
...
dados_sum %>%
  ggplot(aes(x = reorder(uf, total), y = total, fill=uf)) +
  geom_bar(stat = "identity")
```

```{r}
# numero de deputados por estado
dados_sum_dep <- dados_2015 %>%
  group_by(uf) %>%
  summarise(n_deputados = length(unique(nome))) %>%
  rename(estado = uf)

# valor total gasto por estado
dados_sum <- dados_2015 %>%
  group_by(uf) %>%
  summarise(total_estado = sum(valor_liq)) %>%
  rename(estado = uf)

# valor total ganho e gasto por estado
dados_join <- left_join(dados_sum_dep, dados_cota, by = "estado") %>%
  mutate(total_deputado = n_deputados*valor*12) %>%
  left_join(dados_sum, by = "estado")

dados_join %>%
  ggplot(aes(x = total_deputado, y = total_estado)) +
  geom_point(aes(colour=estado))
```

```{r}
cor(dados_join$total_deputado, dados_join$total_estado, method='pearson')
cor(dados_join$total_deputado, dados_join$total_estado, method='kendall')
cor(dados_join$total_deputado, dados_join$total_estado, method='spearman')
```
...
dados_sum_total <- dados_2015 %>%
  group_by(uf) %>%
  summarise(total = sum(valor_liq))

left_join(dados_sum_dep, dados_sum_total, by = "uf") %>%
  ggplot(aes(x = n_deputados, y = total)) +
...
left_join(dados_sum_dep, dados_sum_total, by = "uf") %>%
  mutate(valor = total / (n_deputados*12)) %>%
  rename(estado = uf) %>%
  ggplot(aes(x = reorder(estado, valor), y = valor)) +
