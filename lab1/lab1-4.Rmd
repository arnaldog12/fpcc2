...
De uma maneira geral, __os valores gastos por partido tendem a aumentar ao longo do ano__, como esperado. Porém, ao contrário do imaginado, __não há um aumento de gastos significante perto das eleições__ (meses anteriores a Outubro). Além disso, o gráfico mostra dois pontos que eu considero importante:
...
Vou começar com a análise descritivas das variáveis utilizadas, assim como na pergunta anterior. Vou começar visualizando a distribuição das cotas:
```{r}
dados_cota %>%
...
Também quero saber a quantidade deputados por estado:
```{r, fig.width=12}
...
Agora, para responder a pergunta do exercício, vou analisar a relação entre a cota que o estado recebe e o valor líquido total gasto anual. Eu espero que haja uma forte correlação entre essas variáveis (acima de 0.9 para o coeficiente de Pearson):
```{r, fig.width=12}
dados_join %>%
...
Logo, __os coeficientes também confirmam uma forte relação positiva entre entre a cota anual recebida por estado e o valor líquido total gasto anualmente.__

A títutlo de curiosidade, resolvi plotar o mesmo gráfico novamente, porém comparando o valor total gasto anual pelo número de deputados de um estado:
```{r, fig.width=12}
dados_join %>%
...
O gráfico, como esperado, apresenta um comportamento similar ao anterior, porém agora podemos ver __o valor da cota recebido por deputado (tamanho do círculo) não influencia necessariamente no valor total gasto anual__. Podemos observar que alguns estados que recebem cotas maiores (círculos maiores) se concentram na parte inferior-esquerda do gráfico. Obviamente, isso deve-se ao fato (provavelmente) de serem estados com menos deputados.
...
* O valor da cota recebido por deputado não influencia necessariamente no valor total gasto anual. Ou seja, 
* O gasto médio por deputado de cada estado é sempre menor que a cota recebida;
